...
function setCharge(id, nm, userIdForm, userNameForm) {
	
	var caller_chrg_user_id = ($('#'+userIdForm).val());
	var caller_chrg_user_nm = ($('#'+userNameForm).val());
					        ...
					        <tbody>
					       	...
					       	<c:forEach items="${userList}" var="user" varStatus="state">
								<tr>
									...
									<td>${user.ORG_NM_TEXT}</td>
									<td calss="text-center"><a class="btn btn-success btn-xs " href="javascript:setCharge('${user.USER_ID}', '${user.USER_NM}', '${userIdForm}', '${userNameForm}');" data-user-id="${user.USER_ID}">선택</a></td>
								</tr>
							</c:forEach>
					        </tbody>
						</table>
